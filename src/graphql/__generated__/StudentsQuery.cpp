...
// Constructor
Test__studentstest::Test__studentstest(StudentsQuery * operation, const std::shared_ptr<Test> &inst)
...
std::shared_ptr<Test> deserializers::des_Test__studentstest(const QJsonObject& data, const StudentsQuery * operation){
if (data.isEmpty()){
    return {};
}
...
// Updater
void updaters::update_Test__studentstest(const std::shared_ptr<Test> & inst, const QJsonObject &data, const StudentsQuery * operation)
{
if (!data.value("subject").isNull()){
...
void Test__studentstest::qtgql_replace_concrete(const std::shared_ptr<Test> & new_inst){
    if (new_inst == m_inst){
    return;
    }
...
// Constructor
Student__students::Student__students(StudentsQuery * operation, const std::shared_ptr<Student> &inst)
: m_inst{inst}, ObjectTypeABC
::ObjectTypeABC
(operation)
{
    ...
    m_test = new qtgql::bases::ListModelABC<Test__studentstest *>(this, std::move(init_vec_test));
...
auto new_data = m_inst->get_test();
    ...
    if (i >= prev_len){
            ...
            else{ m_test->replace(i, new Test__studentstest(operation, concrete));
            }
        }
...
std::shared_ptr<Student> deserializers::des_Student__students(const QJsonObject& data, const StudentsQuery * operation){
if (data.isEmpty()){
    return {};
}
...
if (!data.value("test").isNull()){
        ...
        for (const auto& node: data.value("test").toArray()){
            ...
            test_init_vec.push_back(deserializers::des_Test__studentstest(node.toObject(), operation));
...
// Updater
void updaters::update_Student__students(const std::shared_ptr<Student> & inst, const QJsonObject &data, const StudentsQuery * operation)
{
if (!data.value("name").isNull()){
...
[[nodiscard]] const qtgql::bases::ListModelABC<Test__studentstest *> *  Student__students::get_test() const {
...
void Student__students::qtgql_replace_concrete(const std::shared_ptr<Student> & new_inst){
    if (new_inst == m_inst){
    return;
    }
    m_inst->disconnect(this);
...
// Constructor
Query__::Query__(StudentsQuery * operation): ObjectTypeABC
::ObjectTypeABC
(operation){
...
m_operation = operation;
    ...
    m_students = new qtgql::bases::ListModelABC<Student__students *>(this, std::move(init_vec_students));
...
auto new_data = m_inst->get_students();
    ...
    if (i >= prev_len){
            ...
            else{ m_students->replace(i, new Student__students(operation, concrete));
            }
        }
...
// Updater
void updaters::update_Query__(Query * inst, const QJsonObject &data, const StudentsQuery * operation)
{
...
if (!data.value("students").isNull()){
        ...
        for (const auto& node: data.value("students").toArray()){
            ...
            students_init_vec.push_back(deserializers::des_Student__students(node.toObject(), operation));
...
// Query__ Getters
[[nodiscard]] const qtgql::bases::ListModelABC<Student__students *> *  Query__::get_students() const {
