...
std::shared_ptr<Test> des_Test__studentstest(const QJsonObject& data, const StudentsQuery * operation);
std::shared_ptr<Student> des_Student__students(const QJsonObject& data, const StudentsQuery * operation);
};

namespace updaters{
void update_Test__studentstest(const std::shared_ptr<Test> & inst, const QJsonObject &data, const StudentsQuery * operation);
void update_Student__students(const std::shared_ptr<Student> & inst, const QJsonObject &data, const StudentsQuery * operation);
void update_Query__(Query * inst, const QJsonObject &data, const StudentsQuery * operation);
};
...
public:
Test__studentstest(StudentsQuery * operation, const std::shared_ptr<Test> &inst);
...
Q_PROPERTY(const qtgql::bases::ListModelABC<Test__studentstest *> * test READ get_test NOTIFY testChanged);
...
public:
Student__students(StudentsQuery * operation, const std::shared_ptr<Student> &inst);
...
[[nodiscard]] const qtgql::bases::ListModelABC<Test__studentstest *> *  get_test() const;
public:
[[nodiscard]] const QString & __typename() const final{
    return m_inst->__typename();
}
...
Q_PROPERTY(const qtgql::bases::ListModelABC<Student__students *> * students READ get_students NOTIFY studentsChanged);
...
protected:
    void _qtgql_connect_signals();
public:
[[nodiscard]] const qtgql::bases::ListModelABC<Student__students *> *  get_students() const;
public:
[[nodiscard]] const QString & __typename() const final{
    return m_inst->__typename();
}
...
StudentsQuery(): qtgql::bases::OperationHandlerABC(qtgql::bases::GraphQLMessage("query StudentsQuery {""  students {""    name""    test {""      subject""      grade""    }""  }""}")){};
...
UseStudentsQuery(QObject *parent = nullptr): QObject(parent){
    ...
    connect(op_ptr, &StudentsQuery::dataChanged, this, [&]{emit dataChanged();});
    connect(op_ptr, &StudentsQuery::completedChanged, this, [&]{emit completedChanged();});
    connect(op_ptr, &StudentsQuery::operationOnFlightChanged, this, [&]{emit operationOnFlightChanged();});
};
